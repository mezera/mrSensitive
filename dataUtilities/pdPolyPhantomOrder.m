...
% from each coil
[M0, SZ, meanVal, t1,mask] = phantomGetData(nSamples,sampleLocation,smoothkernel);
...
for ii=1:nCoils
    params(:,ii)= pBasis(mask,:) \ M0_v(mask,ii);
end
...
%  std(  M0S_v(:) - M0_v(:) )
lst = M0_v > noiseRange & repmat(mask_v,1 ,nCoils);

percentError = 100*std( (M0S_v(lst) - M0_v(lst)) ./ M0_v(lst));

%percentError = 100*mean(abs( (M0S_v(lst) - M0_v(lst)) ./ M0_v(lst)));
%percentErrorSTD = 100*std(abs( (M0S_v(lst) - M0_v(lst)) ./ M0_v(lst)));
...
%phantomP.percentErrorSTD = percentErrorSTD;
