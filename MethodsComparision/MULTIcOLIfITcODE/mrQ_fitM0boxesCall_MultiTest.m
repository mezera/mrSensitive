function mrQ_fitM0boxesCall_MultiTest(opt_logname,SunGrid,proclass,RunSelectedJob)
...
if SunGrid==1;
    ...
    if (~exist(dirname,'dir')),
        ...
        if proclass==1
            sgerun2('mrQ_CoilPD_gridFit_Multi(opt,jumpindex,jobindex);',sgename,1,1:ceil(length(opt.wh)/jumpindex),[],[],5000);
        else
            sgerun('mrQ_CoilPD_gridFit_Multi(opt,jumpindex,jobindex);',sgename,1,1:ceil(length(opt.wh)/jumpindex),[],[],5000);
            ...
            if length(find(reval)) > 0
                ...
                if proclass==1
                    for kk=1:length(reval)
                        sgerun2('mrQ_CoilPD_gridFit_Multi(opt,jumpindex,jobindex);',[sgename num2str(kk)],1,reval(kk),[],[],5000);
                    end
                else
                    sgerun('mrQ_CoilPD_gridFit_Multi(opt,jumpindex,jobindex);',sgename,1,reval,[],[],5000);
                end
            end
    ...
    mrQ_CoilPD_gridFit_Multi(opt,jumpindex,1);
