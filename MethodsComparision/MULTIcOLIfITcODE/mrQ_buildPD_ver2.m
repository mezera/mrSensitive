...
if notDefined('ErrorTresh')
    if opt.Reg==1 % for T1 regularization
        ErrorTresh=0.01;
...
if notDefined('RepErrTreshold') %the repetition betwwen coils max error
    if opt.Reg==1 % for T1 regularization
        ...
        RepErrTreshold=0.1;
    elseif opt.Reg==2 % for Corralation regularization by simulation we expect more bias then T1. we must allow it or we won't have data to work with
        RepErrTreshold=0.6;
    elseif opt.Reg==3 % for ridge regularization by simulation we expect very strong bias.we must allow it or we won't have data to work with
        RepErrTreshold=0.95;
    else
        RepErrTreshold=1;
    end
...
if opt.Reg~=2 & opt.Reg~=5 % this oupput (X-vlidation ) are not part of the corralation method
