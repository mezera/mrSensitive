function [logname]=mrQ_PD_multicoil_RgXv_GridCall_ver2(outDir,subName,degrees,M0file,T1file,Fitmask,BMfile,outMm,boxSize,pracent_overlap,Coilsinfo,Reg,Init,clobber,mrQ)
%
%   [opt]=mrQ_PD_multicoil_RgXv_GridCalll_ver2(outDir,SunGrid,proclass,subName,degrees,M0file,T1file,BMfile,outMm,boxSize,pracent_overlap,Coilsinfo,clobber)
...
if(~exist('TissueFile','var') || isempty(TissueFile))
    TissueFile = fullfile(outDir,'brainMask.nii.gz');
end
if ~exist(TissueFile,'file') && isfield(mrQ,'outDir')
    TissueFile = fullfile(mrQ.outDir,'brainMask.nii.gz');
end

if (exist(TissueFile,'file'))
 opt.TissueFile = TissueFile;
end
...
if opt.Reg==1
opt.lambda = [1e4 5e3 1e3 5e2 1e2 5e1 2.5e1 1e1 5 1e0 0.5 0] ;%[1e4  1e3 5e2 1e2  1e1  1e0  0] ; %the different wights (lambda) for the T1 regularization we will check the differnt lambda by cross validation.
end
if opt.Reg==3
