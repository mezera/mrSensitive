function  [X_valdationErr,   gEstT, resnorm, FitT, useX, kFold ]=pdX_valdationLoop_RidgeCorr_lsq( lambda1,kFold,M0,pBasis,g0,D,options,RegWeight,TruePD)
...
%     pdX_valdationLoop_RidgeCorr_lsq(lambda1, kFold, M0, ...
...
for ii=1:length(lambda1),
    ...
    for jj=1:kFold
        ...
        % scale the wait to the values we working with. this is needed to asure the
        % whiat size is not depend on the signal level
        D1=D1*sum(M0(FitMask));
        
        % Searching on the gain parameters, G.
   [gEstT(:,:,jj,ii), resnorm(ii,jj)] = ...
       lsqnonlin(@(par) errFitRidgeCorrNestBiLinear_lsq_v1(par,double(M0),...
       double(pBasis), Ncoils,D1,RegWeight,coefdat,FitMask), ...
             double(g0),[],[],options);
        ...
        [FitT(jj,ii).err_X, FitT(jj,ii).err_F] = X_validation_errHoldVoxel_full(gEstT(:,:,jj,ii),M0,pBasis,nVoxels,Ncoils,FitMask,Xmask,TruePD);
