function OutPut = pdBiLinearRidgeFit_XV(M0_v,pBasis,Lambda,maxLoops,sCriterion,PD,Mask,D)
% OutPut = pdBiLinearRidgeFit_XV(M0_v,pBasis,Lambda,maxLoops,sCriterion,PD,Mask,D)
%
...
if notDefined('Mask'), Mask=logical(M0_v);end
...
while tryagain == 1
    ...
    g = RidgeRegressCoilfit_XV(PD, Lambda, M0_v, pBasis,D,Mask);
    
    % Calculate the coil gain of each coil over space from the estimate
    % Calculate PD for each coil
    [PDn, Gn] = pdEstimate_XV(M0_v, pBasis, g,Mask);
